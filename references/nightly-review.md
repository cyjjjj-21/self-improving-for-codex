...
When the environment runs automations in isolated per-`cwd` executions, prefer this pattern for write-side work:

- use one root `cwd`
- keep the real write-side nightly pipeline outside the automation sandbox, for example with a macOS `launchd` job that calls `scripts/launchd_night_memory_pipeline.py`
- let that wrapper publish a canonical JSON status file and precomputed summary artifact
